...
def test_local_executor_run_case_reports(monkeypatch):
    ...
    ok = exe._run_test_case(session=None, test_run_id="r1", test_case_id="tc1", steps=steps_fail)
    assert ok is False
    ...
    ok2 = exe._run_test_case(session=None, test_run_id="r1", test_case_id="tc2", steps=steps_ok)
    assert ok2 is True
